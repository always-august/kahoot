...
export default function QuizEntryPage() {
  ...
  const join = (e: React.FormEvent) => {
    ...
    if (c) router.push(`/quiz/play?room=${encodeURIComponent(c)}`);
  };
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col justify-center px-5 py-[clamp(1rem,4vh,3.5rem)]">
      ...
      <h1 className="mb-1 text-3xl font-bold text-ink-900">실시간 퀴즈</h1>
      <p className="mb-8 text-ink-700">방을 만들거나 코드로 참여하세요.</p>

      <Link href="/quiz/host" className="btn-primary btn-lg mb-6 w-full">
        방 만들기 (방장)
      </Link>
